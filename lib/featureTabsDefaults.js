...
export function resolveFeatureTabsProps(props) {
  ...
  const firstTabId = String(tabs[0]?.id || '').trim();
  /** Inspector "Default active tab" sets defaultTabExplicit; otherwise live/builder open on first tab. */
  const defaultTabExplicit = p.defaultTabExplicit === true;
  const requestedActive = String(p.activeTabId || firstTabId).trim();
  const activeTabId = defaultTabExplicit ? requestedActive : firstTabId;
  const validActive = tabs.some((t) => t.id === activeTabId) ? activeTabId : firstTabId;
  const imageFitRaw = String(p.imageFit || 'contain').trim().toLowerCase();
  /** Infographic panels: contain shows full artwork on builder, preview, and live (cover crops). */
  const imageFit = imageFitRaw === 'fill' ? 'fill' : 'contain';
  ...
  const panelModeRaw = String(p.panelMode || 'fields').trim();
  const panelMode = panelModeRaw === 'elements' ? 'elements' : 'fields';
  return { tabs, activeTabId: validActive, imageFit, imageHeightPx, tabAlign, panelMode };
}
 ...
 */
/** Stable JSON fingerprint for skip-no-op tab node saves. */
export function featureTabsPropsFingerprint(props) {
  const resolved = resolveFeatureTabsProps(props);
  const p = props && typeof props === 'object' ? props : {};
  return JSON.stringify({
    tabs: resolved.tabs,
    activeTabId: resolved.activeTabId,
    defaultTabExplicit: p.defaultTabExplicit === true,
    tabAlign: resolved.tabAlign,
    panelMode: resolved.panelMode,
    imageFit: resolved.imageFit,
    imageHeightPx: resolved.imageHeightPx,
