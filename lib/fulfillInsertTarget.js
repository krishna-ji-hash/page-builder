import { featureTabPanelStackProps } from './featureTabPanels.js';
...
export async function fulfillInsertTarget(target, createNodeRequest) {
  ...
  if (target.createFeatureTabPanelStack && target.tabsNodeId != null) {
    const stack = await createNodeRequest({
      nodeType: 'stack',
      parentNodeId: target.tabsNodeId,
      displayName: 'Tab panel',
      props: featureTabPanelStackProps(target.tabId),
      positionIndex:
        Number.isInteger(target.insertIndex) && target.insertIndex >= 0 ? target.insertIndex : 0,
    });
    if (!stack?.id) throw new Error('Failed to create tab panel stack.');
    parentId = stack.id;
    insertIndex = 0;
