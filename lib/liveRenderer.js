...
export function renderNode(node, key, options = {}) {
  ...
  const pathPrefixOpts = { currentPath: currentPath || '' };
  const prefixPath = (rawPath) =>
    rawPath && typeof rawPath === 'string' && options.projectSlug
      ? prefixRelativeAppPath(rawPath, options.projectSlug, pathPrefixOpts)
      : rawPath;
  ...
  if (node.nodeType === 'row' || node.nodeType === 'column' || node.nodeType === 'stack') {
    ...
    const useSectionContentShell =
      node.nodeType === 'row' && shouldWrapRootSectionContent(sectionContentMode, isRootContentRow);
    let rowRenderCss = cssWithSectionContent;
    let sectionInnerCss;
    if (useSectionContentShell) {
      const split = splitRowCssForSectionContentShell(cssWithSectionContent);
      rowRenderCss = split.sectionCss;
      sectionInnerCss = split.innerCss;
    }
    const containerClass = useSectionContentShell
      ? appendSectionFullBgClasses([
    ...
    return (
      <Container
        ...
        {...hit}
      >
        {useSectionContentShell ? (
          <div className={SECTION_INNER_CLASS} style={sectionInnerCss}>
            {rowBody}
          </div>
        ) : (
          rowBody
  ...
  if (node.nodeType === 'menu') {
    ...
    let items = useProjectPages
      ...
      : Array.isArray(node.props?.items)
        ...
        : [];
    if (!useProjectPages) {
      const { items: normalized } = normalizeMenuItems(items);
      items = normalized;
      if (options.projectSlug) {
        items = prefixMenuItemsWithProjectSlug(items, options.projectSlug, currentPath || '');
      }
    }
