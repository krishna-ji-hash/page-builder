...
/** Drop `gap` when `columnGap` / `rowGap` are set (React warns on mixed shorthand + longhand). */
export function sanitizeGapShorthandConflict(css) {
  if (!css || typeof css !== 'object') return css;
  const hasGapLonghand =
    (css.columnGap != null && css.columnGap !== '') ||
    (css.rowGap != null && css.rowGap !== '');
  if (!hasGapLonghand) return css;
...
export function styleToCss(style, siteTheme) {
  ...
  const ixVars = interactionInlineStyleVars(style);
  const ixAnim = animationCssFromInteractions(style);
...
function mergeDeviceLayers(base = {}, override = {}) {
  return {
    ...
    interactions: { ...(base.interactions || {}), ...(override.interactions || {}) },
