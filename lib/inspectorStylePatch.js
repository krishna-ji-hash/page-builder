import { isCssGradientImage, backgroundImageUrlForInspector } from './colorInputUtils.js';
...
function buildBackgroundPatch(nextForm, key, currentBackground = {}) {
  const rawBgImg = backgroundImageUrlForInspector(nextForm.bgImageUrl);
  const hasUrlImage = Boolean(rawBgImg);
  const hadGradient = isCssGradientImage(currentBackground?.backgroundImage);
  ...
  if (hasUrlImage) {
    patch.backgroundImage = rawBgImg;
    patch.backgroundSize = String(nextForm.bgSize || 'cover');
    patch.backgroundPosition = String(nextForm.bgPosition || 'center center');
    patch.backgroundRepeat = String(nextForm.bgRepeat || 'no-repeat');
    patch.meta = {
...
export function buildInspectorStylePatch(key, nextForm, ctx) {
  ...
  if (BG_KEYS.has(key)) {
    const currentBackground = getDeviceStyle(
      normalizeResponsiveStyle(selectedNode?.style_json || {}, {
        nodeType: selectedNode?.nodeType,
        siteTheme: ctx.siteTheme,
      }),
      ctx.device
    )?.background;
    patch.colors = { backgroundColor: nextForm.bgColor };
    patch.background = buildBackgroundPatch(nextForm, key, currentBackground || {});
    return { patch };
  }
