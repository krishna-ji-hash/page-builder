 ...
 * @param {{ neutralizeHardcodedBodyTextColors?: boolean }} [options] — forwarded to `sanitizeRichHtml` (dark site preset).
 */
export function sanitizeInlineLeafHtml(html, options = {}) {
  if (typeof html !== 'string' || !html.trim()) return '';
  return sanitizeRichHtml(html, { emptyReturn: '', ...options }).trim();
}
