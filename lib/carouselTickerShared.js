...
/**
 * @param {string} variantKey - `ticker` | `marquee` | …
 * @param {string} [scrollDirectionRaw]
 * @returns {{ isMarqueeVariant: boolean, row1TrackClass: string, row2TrackClass: string }}
 */
export function resolveTickerScrollClasses(variantKey, scrollDirectionRaw) {
    ...
    const row2TrackClass =
      scrollDirection === 'opposite'
        ...
        : scrollDirection === 'right'
          ? 'live-carousel__ticker-track--rtl'
          : 'live-carousel__ticker-track--ltr';
    return { isMarqueeVariant: false, row1TrackClass, row2TrackClass };
  }
  ...
  const row2TrackClass = row1TrackClass;
