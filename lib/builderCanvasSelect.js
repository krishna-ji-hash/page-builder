...
const TAB_HERO_CONTENT_SELECTOR =
  '[data-tab-hero], .live-tab-hero, .live-tab-hero__nav, .live-tab-hero__stage, .live-tab-hero__card, .live-tab-hero__editable, [data-bld-feature-tab-field]';

const STATS_COUNTER_CONTENT_SELECTOR = '[data-stats-counter], .live-stats-counter, .live-stats-counter__item';

const GRID_BLOCK_CONTENT_SELECTOR = '[data-grid-block], .bld-el-grid, .bld-el-grid__cell, .bld-el-grid__title, .bld-el-grid__text';
...
export function resolveBuilderCanvasSelectTarget(event, currentNodeId) {
  ...
  const tabHero = target.closest(TAB_HERO_CONTENT_SELECTOR);
  if (tabHero) {
    const shell = tabHero.closest('[data-bld-node]');
    const nestedId = shell?.getAttribute('data-bld-node');
    if (nestedId && String(nestedId) !== String(currentNodeId)) {
      return nestedId;
    }
  ...
  const statsCounter = target.closest(STATS_COUNTER_CONTENT_SELECTOR);
  if (statsCounter) {
    const shell = statsCounter.closest('[data-bld-node]');
    const nestedId = shell?.getAttribute('data-bld-node');
    if (nestedId && String(nestedId) !== String(currentNodeId)) {
      return nestedId;
    }
  ...
  const gridBlock = target.closest(GRID_BLOCK_CONTENT_SELECTOR);
  if (gridBlock) {
    const shell = gridBlock.closest('[data-bld-node]');
    const nestedId = shell?.getAttribute('data-bld-node');
    if (nestedId && String(nestedId) !== String(currentNodeId)) {
      return nestedId;
    }
