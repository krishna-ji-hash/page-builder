...
import { liveDocRootRowSpacingVars } from '@/lib/liveDocSectionSpacing';
import { rowSectionStripDataAttrs } from '@/lib/livePageCssVars';
...
export function renderNode(node, key, options = {}) {
  ...
  const childOptions = { ...options, device, isLiveDocRootRow: false };

  if (!node?.nodeType) return null;
  const deviceStyleThemed = themedDeviceStyle(node, device, options);
  const mergedBase = mergeDeviceStyleWithTypeDefaults(node.nodeType, deviceStyleThemed, { treeNode: node });
  ...
  const liveRootGapBeforeAttrs =
    ...
    Object.prototype.hasOwnProperty.call(rootSpacingVars, '--live-row-gap-before')
      ...
      : {};
  const stripAttrs =
    node.nodeType === 'row' ? rowSectionStripDataAttrs(Boolean(options.isLiveDocRootRow), rowMeta) : {};
