...
export function sanitizeHeaderRowCss(css, rowMeta) {
  ...
  out.marginTop = clampNonNegativeMargin(out.marginTop);
  out.marginBottom = clampNonNegativeMargin(out.marginBottom);
  if (typeof out.margin === 'string' && out.margin.trim().startsWith('-')) {
    out.margin = '0px';
  }
  if (typeof out.transform === 'string' && /translate/i.test(out.transform)) {
