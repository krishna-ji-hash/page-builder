...
export function resolveSectionBackgroundIsLight({ css, deviceStyle, siteTheme }) {
  ...
  for (const c of list) {
    ...
    const resolved = resolveSiteThemeColorRef(raw, theme);
    const probe = isTransparentColor(resolved) ? raw : resolved;
 ...
 * @param {object} siteTheme
 */
export function containerPaintsDark(node, device, siteTheme) {
  ...
  return resolveSectionBackgroundIsLight({ css: null, deviceStyle: themed, siteTheme }) === false;
}
