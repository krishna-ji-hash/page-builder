...
export function mergeDeviceStyleWithTypeDefaults(nodeType, deviceStyle = {}, options = {}) {
  ...
  if (nodeType === 'column') {
    ...
    return {
      ...
      spacing: mergeSpacing(spacing, { padding: '0 0 24px' }),
    };
  }
  ...
  if (nodeType === 'button') {
    const merged = mergeLayout(layout, {
      ...
      alignSelf: layout.alignSelf ?? 'flex-start',
      boxSizing: 'border-box',
      flexShrink: 0,
      minWidth: 0,
      maxWidth: '100%',
  ...
  if (nodeType === 'tabs' || nodeType === 'tab_hero' || nodeType === 'stats_counter') {
    return {
      ...deviceStyle,
      ...
      size: mergeSize(size, {
        width: size.width == null || size.width === '' ? '100%' : size.width,
        height: size.height ?? 'auto',
      }),
    };
  }
  ...
  if (nodeType === 'spacer') {
    const heightPx = Number(options?.treeNode?.props?.heightPx) > 0 ? Number(options.treeNode.props.heightPx) : 48;
    return {
      ...deviceStyle,
      layout: mergeLayout(layout, { display: 'block', flexShrink: 0, boxSizing: 'border-box' }),
      size: mergeSize(size, { width: '100%', height: `${heightPx}px` }),
    };
  }

  if (nodeType === 'divider') {
    const orientation =
      options?.treeNode?.props?.orientation === 'vertical' ? 'vertical' : 'horizontal';
    const thickness = Number(options?.treeNode?.props?.thicknessPx) > 0 ? Number(options.treeNode.props.thicknessPx) : 2;
    ...
    return {
      ...
      size: mergeSize(size, {
        ...(isVertical
          ? {
              width: size.width ?? `${thickness}px`,
              height: size.height ?? '48px',
              minHeight: size.minHeight ?? '24px',
            }
          : {
              width: size.width ?? '100%',
              height: size.height ?? `${thickness}px`,
            }),
      }),
    };
  }
