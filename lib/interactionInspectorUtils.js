...
import { cleanParallax } from './interactionParallax.js';
...
function cleanAnimation(anim = {}) {
  ...
  out.trigger = allowed.has(trigger) ? trigger : 'on-enter-viewport';
...
export function pruneInteractions(ix = {}) {
  ...
  const parallax = cleanParallax(ix.parallax);
...
export function patchInteractionGroup(currentIx = {}, group, key, value) {
  ...
  } else if (group === 'parallax' && key === 'enabled') {
    nextGroup[key] = Boolean(value);
  } else if (group === 'parallax' && key === 'speed') {
    const n = parseFiniteNumber(value);
    if (n != null && n >= 0) nextGroup[key] = n;
  } else if (group === 'parallax' && key === 'direction') {
    nextGroup[key] = String(value || 'vertical-up');
  ...
  const cleaned =
    group === 'animation'
      ? cleanAnimation(nextGroup)
      : group === 'parallax'
        ? cleanParallax(nextGroup)
        : cleanGroup(nextGroup);
...
/** Enable entrance animation in one atomic patch (avoids sequential preset/trigger wipes). */
export function enableEntranceAnimation(currentIx = {}, preset, extras = {}) {
  const prev = pruneInteractions(currentIx);
  const next = { ...prev };
  const anim = cleanAnimation({
    preset: normalizeAnimationPreset(preset),
    trigger: 'on-enter-viewport',
    duration: 0.6,
    ...(extras && typeof extras === 'object' ? extras : {}),
  });
  if (anim) next.animation = anim;
  else delete next.animation;
  return pruneInteractions(next);
}

/** Merge partial interaction groups in one atomic update. */
export function mergeInteractionsPatch(currentIx = {}, patch = {}) {
  const prev = pruneInteractions(currentIx);
  const next = { ...prev };
  for (const group of ['hover', 'pressed', 'focus', 'animation', 'parallax']) {
    if (!patch[group] || typeof patch[group] !== 'object') continue;
    const merged = { ...(prev[group] || {}), ...patch[group] };
    let cleaned = null;
    if (group === 'animation') cleaned = cleanAnimation(merged);
    else if (group === 'parallax') cleaned = cleanParallax(merged);
    else cleaned = cleanGroup(merged);
