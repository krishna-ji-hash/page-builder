...
function themedDeviceStyle(node, device, options) {
  ...
  const merged = withResolvedLayoutGap(mergeNodeStyleWithSiteTheme(withPreset, theme, node.nodeType), theme);
...
export function renderNode(node, key, options = {}) {
  ...
  options._ixClass = interactionPresentationClass(mergedBase);
