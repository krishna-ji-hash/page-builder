...
export function generateSchemaJsonLd({
  ...
  const description = str(pageSeo.description || projectSeo.defaultDescription);
  const url = str(canonical);
  const siteName = str(projectSeo.siteTitle || projectSeo.siteName);
  const image = str(pageSeo.ogImage || projectSeo.defaultOgImage);

  const org = {
    '@type': 'Organization',
    name: str(projectSeo.companyName || siteName),
    url: str(projectSeo.canonicalDomain || url),
    ...(image ? { logo: image } : {}),
  };
