...
import { normalizeHtmlBlockCssInput, splitHtmlBlockStyleFromHtml } from '@/lib/htmlBlockCss';

export const HTML_BLOCK_FORM_KEYS = new Set(['htmlBlockContent', 'htmlBlockCss']);

export function isHtmlBlockFormKey(key) {
  return HTML_BLOCK_FORM_KEYS.has(key);
}

export function advancedElementFormFromPropsBatch2(props = {}, pickPending) {
  const pick = (fieldKey, val) => (typeof pickPending === 'function' ? pickPending(fieldKey, val) : val);
  return {
    ...
    htmlBlockContent: pick('htmlBlockContent', props.html ?? ''),
    htmlBlockCss: pick('htmlBlockCss', props.css ?? ''),
...
export function advancedElementPatchFromFormKeyBatch2(nodeType, key, value) {
  switch (nodeType) {
    ...
    case 'countdown_timer':
      ...
      break;
    case 'html_block':
      if (key === 'htmlBlockContent') {
        const { html } = splitHtmlBlockStyleFromHtml(String(value ?? ''));
        return { patch: { html } };
      }
      if (key === 'htmlBlockCss') return { patch: { css: normalizeHtmlBlockCssInput(String(value ?? '')) } };
      break;
    case 'code_block':
