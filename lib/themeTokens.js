...
/** Copy site theme brand colors into the active token palette (admin + publish parity). */
export function syncThemeTokenPaletteFromSiteTheme(siteTheme, themeTokens) {
  const site = normalizeSiteTheme(siteTheme);
  const mode = site.presetId === 'dark' ? 'dark' : 'light';
  const aligned = alignThemeTokensWithSiteTheme(site, themeTokens);
  const withPalettes = hasModePalettes(aligned)
    ? aligned
    : { ...aligned, ...createModePalettesFromFlat(aligned) };
  const palette = withPalettes[mode] || {};
