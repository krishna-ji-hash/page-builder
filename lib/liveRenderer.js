...
export function renderNode(node, key, options = {}) {
  ...
  if (node.nodeType === 'image') {
    ...
    let figureStyle =
      imageFitMode(node.props?.imageFit) === 'contain' || isNarrowViewport
        ? mergeImageFigureStyleForContain(css)
        : css;
    ...
    const imgStyle = clampImgStyleForViewport(imgStyleBase, { imageHeightPx, device });
