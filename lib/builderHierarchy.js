import { PDP_BLOCK_NODE_TYPES } from './pdpElementRegistry.js';
...
export const BLOCK_TYPES = new Set([
  ...
  'feature_list',
  'table_pro',
  /* PDP widgets (lib/pdpElementRegistry.js) */
  ...PDP_BLOCK_NODE_TYPES,
]);
...
export function assertValidNodeHierarchy(childNodeType, parentNodeType) {
  ...
  if (normalizedParent === 'tabs') {
    if (normalizedChild !== 'stack') {
      throw new Error('Invalid hierarchy: only stack panel roots may be placed inside feature tabs');
    }
    return;
  }
