...
const BUTTON_ON_GRADIENT_TEXT = 'var(--token-button-text, #ffffff)';

function isGradientPaint(raw) {
  return /gradient\s*\(/i.test(String(raw || ''));
}

/**
 * Glass preset + gradient preset merges can leave rgba(255,255,255,0.55) under a gradient
 * and dark body text on buttons — preview/live render that literally; strip the conflict.
 * @param {Record<string, unknown>} css
 * @param {{ nodeType?: string }} [opts]
 */
export function sanitizeGradientPaintCss(css, opts = {}) {
  if (!css || typeof css !== 'object') return css;
  if (!css.backgroundImage || !isGradientPaint(css.backgroundImage)) return css;

  const next = { ...css };
  if (next.backgroundColor && shouldStripPastedBackgroundCssColor(next.backgroundColor)) {
    next.backgroundColor = 'transparent';
  }
  if (next['--node-bg'] && shouldStripPastedBackgroundCssColor(next['--node-bg'])) {
    next['--node-bg'] = 'transparent';
  }

  if (opts.nodeType === 'button') {
    const text = next.color || next['--node-text'];
    if (text && shouldStripNeutralDarkCssColor(text)) {
      next.color = BUTTON_ON_GRADIENT_TEXT;
      next['--node-text'] = BUTTON_ON_GRADIENT_TEXT;
    }
  }
  return next;
}
...
export function coerceSectionAwareTextColor(raw, opts = {}) {
  ...
  if (opts.nodeType === 'button' && opts.style && hasAuthoredControlContrast(opts.style)) {
    return coerceTokenRef(raw);
  }
  const neutralizeDark =
    Boolean(opts.darkContentMode) || opts.sectionTone === 'dark';
  if (neutralizeDark && shouldStripNeutralLightCssColor(s)) return SECTION_AWARE_TEXT;
  if (neutralizeDark && shouldStripNeutralDarkCssColor(s)) {
    return opts.darkContentMode ? 'var(--color-text)' : SECTION_AWARE_TEXT;
  }
  return coerceTokenRef(raw);
}
...
function designTokenVars(style, siteTheme, opts = {}) {
  ...
  const textColor = coerceSectionAwareTextColor(textColorRaw, { ...opts, style });
  const bg =
    style.colors?.background ||
    style.colors?.backgroundColor ||
    style.background?.backgroundColor;
  const bgImageRaw = style.background?.backgroundImage;
  const bgImage =
    bgImageRaw && String(bgImageRaw).trim() !== '' && String(bgImageRaw).trim() !== 'none'
      ? String(bgImageRaw).trim()
      : '';
  ...
  if (bgImage) {
    out['--btn-bg-image'] = bgImage;
    out['--btn-bg-size'] = style.background?.backgroundSize || 'cover';
    out['--btn-bg-position'] = style.background?.backgroundPosition || 'center';
    out['--btn-bg-repeat'] = style.background?.backgroundRepeat || 'no-repeat';
  }
...
export function styleToCss(style, siteTheme, opts = {}) {
  ...
  const out = {
    ...
    String(style.background.backgroundImage).trim() !== '' &&
    String(style.background.backgroundImage).trim() !== 'none'
      ? {
  ...
  if (opts?.nodeType && shouldApplySectionContrast(opts.nodeType, result, style)) {
    result = mergeLiveSectionContrastVars(result, style, theme);
  }
  return sanitizeGradientPaintCss(result, opts);
}
...
function mergeInteractionLayers(base, override) {
  ...
  const out = { ...(base || {}) };
  for (const key of ['hover', 'pressed', 'active', 'focus', 'animation', 'parallax']) {
