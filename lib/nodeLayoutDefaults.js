import { DEFAULT_SITE_THEME, normalizeSiteTheme, themeSpacingPx } from '@/lib/siteDesignTheme';
 ...
 */
export function mergeDeviceStyleWithTypeDefaults(nodeType, deviceStyle = {}) {
  ...
  if (nodeType === 'stack') {
    const gapFromSpacing =
      spacing.gap != null && spacing.gap !== ''
        ? Number.parseFloat(String(spacing.gap).replace(/px/gi, '').trim())
        : null;
    const resolvedGap =
      layout.gap !== undefined && layout.gap !== null && layout.gap !== ''
        ? layout.gap
        : Number.isFinite(gapFromSpacing)
          ? gapFromSpacing
          : 12;
    const { gap: _dropLegacyStackGap, ...spacingRest } = spacing;
    const explicitDir = layout.flexDirection;
    const implicitDir =
      explicitDir != null && explicitDir !== '' && explicitDir !== undefined ? explicitDir : 'row';
