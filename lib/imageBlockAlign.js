...
export function resolveImageShellAlignStyle(slot, parentFlexDirection = 'column') {
  ...
  if (['top', 'bottom'].includes(align)) {
    return resolveImageVerticalAlign(align, parentFlexDirection);
  }
  return {};
}
...
export function splitImageNodeCss(css) {
  ...
  return {
    shell: Object.keys(shell).length ? shell : undefined,
    figure: Object.keys(figure).length ? figure : undefined,
  };
}
...
export function buildImageAlignStylePatch(
  ...
  const box = parseMarginBoxForPatch(deviceStyle?.spacing?.margin);
  const size = {};
