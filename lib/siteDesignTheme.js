import { nodeHasUserFontOverride, stripFontFamilyFromStyleLayer } from './projectBrand.js';
...
export function siteThemeDefaultsForNodeType(siteTheme, nodeType) {
  ...
  /** Rows set `--live-section-fg` from resolved section background; explicit node colors still win. */
  const bodyFg = 'var(--live-section-fg, var(--color-text))';
  ...
  switch (nodeType) {
    ...
    case 'paragraph': {
      const bodyMuted = 'var(--live-section-muted, var(--color-muted))';
      return {
        typography: {
          fontFamily: fam,
          color: bodyMuted,
 ...
 * @param {string} nodeType
 */
function rowHasUserBackgroundPaint(deviceStyle) {
  const bg = deviceStyle?.background;
  if (!bg || typeof bg !== 'object') return false;
  return Boolean(
    (bg.backgroundImage != null && String(bg.backgroundImage).trim() !== '') ||
      (bg.backgroundColor != null && String(bg.backgroundColor).trim() !== '')
  );
}

/**
 * @param {object} [options]
 * @param {object} [options.treeNode] — when set, template fontFamily in style_json is ignored unless `props.meta.userFontOverride`
 */
export function mergeNodeStyleWithSiteTheme(deviceStyle, siteTheme, nodeType, options = {}) {
  if (!siteTheme || typeof siteTheme !== 'object') return deviceStyle || {};
  let style = deviceStyle || {};
  if (!nodeHasUserFontOverride(options.treeNode)) {
    style = stripFontFamilyFromStyleLayer(style);
  }
  const defaults = siteThemeDefaultsForNodeType(siteTheme, nodeType);
  if (nodeType === 'row' && rowHasUserBackgroundPaint(style)) {
    const { background: _skipRowBg, ...rest } = defaults;
    return deepMergePreferDefined(style, rest);
  }
  return deepMergePreferDefined(style, defaults);
}
