...
import {
  headerRevealBarFromBehavior,
  headerRevealBarToCssVars,
  normalizeHeaderRevealBarStyle,
} from './headerRevealBarStyle.js';
...
export function normalizeHeaderBehavior(raw, fallback = DEFAULT_HEADER_BEHAVIOR) {
  ...
  return {
    ...
    revealBar: normalizeHeaderRevealBarStyle(b.revealBar ?? base.revealBar),
...
export function headerBehaviorMetaPatch(prevMeta = {}, patch = {}) {
  ...
  const { revealBar: revealBarPatch, ...behaviorPatch } =
    patch && typeof patch === 'object' ? patch : {};
  const next = normalizeHeaderBehavior({
    ...prev,
    ...behaviorPatch,
    ...(revealBarPatch && typeof revealBarPatch === 'object'
      ? { revealBar: { ...prev.revealBar, ...revealBarPatch } }
      : {}),
  });
  return {
    ...prevMeta,
...
/** Merge reveal bar CSS vars + data attrs onto header row render props. */
export function headerBehaviorRenderExtras(behavior) {
  if (!behavior) return { style: {}, attrs: {} };
  const bar = headerRevealBarFromBehavior(behavior);
  const type = String(behavior.type || '').toLowerCase();
  if (type !== 'mainreveal' && type !== 'revealonscroll') {
    return { style: {}, attrs: {} };
  }
  const attrs = {};
  if (bar.maxWidthPct > 0 && bar.maxWidthPct < 100) {
    attrs['data-header-reveal-bar-contained'] = 'true';
  }
  return {
    style: headerRevealBarToCssVars(bar),
    attrs,
  };
}
