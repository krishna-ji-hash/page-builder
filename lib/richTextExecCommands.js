...
/** Saved toolbar range is a non-collapsed highlight inside `root`. */
export function hasSavedWordSelectionInRoot(root) {
  if (!root) return false;
  const range = savedSelectionByRoot.get(root);
  if (!range) return false;
  try {
    return !range.collapsed && root.contains(range.commonAncestorContainer);
  } catch (_) {
    return false;
  }
}
...
export function applyRichColorInRoot(root, command, color, options = {}) {
  ...
  // Color picker / toolbar focus collapses the live selection — use saved word highlight.
  if ((!range || !rangeInRoot(range) || range.collapsed) && !selectAllIfCollapsed) {
    const saved = getSavedRichTextSelection(root);
