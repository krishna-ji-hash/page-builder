...
export function advancedElementPatchFromFormKeyBatch2(nodeType, key, value) {
  switch (nodeType) {
    ...
    case 'lottie_animation':
      ...
      break;
    case 'logo_block':
      if (key === 'logoSrc') return { patch: { src: value } };
      if (key === 'logoAlt') return { patch: { alt: value } };
      if (key === 'logoHref') return { patch: { href: value } };
      if (key === 'logoWidthPx') return { patch: { widthPx: Math.max(48, Math.min(400, Number(value) || 160)) } };
      break;
    case 'feature_list':
