...
export function featureTabsInspectorFormFromProps(props, pickPending) {
  ...
  return {
    ...
    featureTabsActiveId: String(
      p.defaultTabExplicit === true
        ? p.activeTabId || (Array.isArray(p.tabs) && p.tabs[0]?.id ? p.tabs[0].id : '') || ''
        : (Array.isArray(p.tabs) && p.tabs[0]?.id ? p.tabs[0].id : '') || ''
    ),
    featureTabsImageFit: String(p.imageFit || 'contain'),
    featureTabsImageHeightPx: String(Number(p.imageHeightPx) || 360),
    featureTabsTabAlign: String(p.tabAlign || 'center'),
    featureTabsPanelMode: String(p.panelMode || 'fields'),
