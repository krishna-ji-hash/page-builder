...
export function stripNeutralBodyColorsFromStyleAttr(styleAttr) {
  ...
  for (const part of parts) {
    ...
    if ((prop === 'color' || prop === '-webkit-text-fill-color') && shouldStripNeutralDarkCssColor(val)) continue;
    kept.push(part);
  }
 ...
 */
export function neutralizeLeafTextCssObject(css) {
  if (!css || typeof css !== 'object') return css;
  let changed = false;
  const next = { ...css };
  if (next.color != null && next.color !== '' && shouldStripNeutralDarkCssColor(String(next.color))) {
    next.color = 'var(--color-text)';
    changed = true;
  }
  const nt = next['--node-text'];
  if (nt != null && nt !== '' && shouldStripNeutralDarkCssColor(String(nt))) {
    next['--node-text'] = 'var(--color-text)';
    changed = true;
  }
...
function ensureHtmlHooks() {
  ...
  DOMPurify.addHook('afterSanitizeAttributes', (node) => {
    if (!neutralizeHardcodedBodyTextColorsActive) return;
