...
import { normalizePublicSiteHref, prefixMenuItemsWithProjectSlug } from '@/lib/projectPathPrefix';
import { isPlaceholderNavHref, resolvePlaceholderNavHref } from '@/lib/seo/navLinkResolver';

function publicPathOpts(options = {}) {
  return {
    currentPath: options.currentPath,
    publicSite: options.publicSite === true,
  };
}
...
import FaqFullPage from '@/components/runtime/FaqFullPage';
import BlogFullPage from '@/components/runtime/BlogFullPage';
...
import { getBlogSectionDef } from '@/lib/blogSectionRegistry';
import { getBlogDetailSectionDef } from '@/lib/blogDetailSectionRegistry';
...
import { decodePlainTextEntities, isProbablyInlineHtml, sanitizeInlineLeafHtml } from '@/lib/inlineTextHtml';
...
function renderButtonContent(node) {
  const text = decodePlainTextEntities(node.props?.text) || 'Button';
...
export function renderNode(node, key, options = {}) {
  ...
  if (node.nodeType === 'row' || node.nodeType === 'column' || node.nodeType === 'stack') {
    ...
    const tplRoleAttrs = nodeMeta.tplRole
      ? {
          'data-tpl-role': String(nodeMeta.tplRole),
          ...(nodeMeta.tplTheme ? { 'data-tpl-theme': String(nodeMeta.tplTheme) } : {}),
        }
      : {};
    ...
    return (
      ...
      >
        <RichTextLeaf
          ...
          style={css}
          className={leafClass}
          tag={node.nodeType === 'paragraph' ? 'div' : undefined}
          sanitizeOptions={{
            ...sanitizeOptions,
  ...
  if (node.nodeType === 'image') {
    const brandLogoCtx = {
      tree: options.builderTree,
      inSiteHeader:
        Boolean(options.insideSiteHeaderRow) ||
        (Array.isArray(options.builderTree) &&
          node.id != null &&
          nodeIsInsideSiteHeader(options.builderTree, node.id)),
    };
    const isBrandLogo = nodeLooksLikeBrandLogo(node, brandLogoCtx);
    ...
    const placeholderSrc = '/builder-placeholder.svg';
    let src = String(node.props?.src || '').trim();
    if (!isBrandLogo) {
      const legacyLogoUrl = String(node.props?.lightLogoUrl || '').trim();
      if ((!src || src === placeholderSrc) && legacyLogoUrl) src = legacyLogoUrl;
    }
    ...
    const href =
      hrefRaw && typeof hrefRaw === 'string'
        ? normalizePublicSiteHref(hrefRaw, options.projectSlug, publicPathOpts(options))
        : hrefRaw;
  ...
  if (node.nodeType === 'faq_full_page') {
    const faqPageProps = resolveFaqFullPageProps(node.props);
    const faqPageCss = sanitizeCompoundWidgetShellCss(css);
    const bCanvas = options.builderCanvas;
    const faqPageBuilder = Boolean(bCanvas?.faqFullPage?.onOpenItemChange);
    const inner = (
      <FaqFullPage
        key={key}
        style={faqPageCss}
        {...faqPageProps}
        builderMode={faqPageBuilder}
        builderEditable={faqPageBuilder && !bCanvas?.sectionEditLocked}
        onOpenItemChange={bCanvas?.faqFullPage?.onOpenItemChange}
        onPatchItem={bCanvas?.faqFullPage?.onPatchItem}
        onPatchContent={bCanvas?.faqFullPage?.onPatchContent}
        onAddItem={bCanvas?.faqFullPage?.onAddItem}
      />
    );
    return options.builderDataAttr ? (
      <div key={key} className={leafClassName('', options)} style={{ width: '100%', minWidth: 0 }} {...hit}>
  ...
  if (node.nodeType === 'blog_full_page') {
    const blogPageProps = resolveBlogFullPageProps(node.props);
    const blogPageCss = sanitizeCompoundWidgetShellCss(css);
    const bCanvas = options.builderCanvas;
    const blogPageBuilder = Boolean(bCanvas?.blogFullPage?.onPatchContent);
    const inner = (
      <BlogFullPage
        key={key}
        style={blogPageCss}
        {...blogPageProps}
        builderMode={blogPageBuilder}
        builderEditable={blogPageBuilder && !bCanvas?.sectionEditLocked}
        onPatchContent={bCanvas?.blogFullPage?.onPatchContent}
        onPatchPost={bCanvas?.blogFullPage?.onPatchPost}
        onAddPost={bCanvas?.blogFullPage?.onAddPost}
      />
    );
    return options.builderDataAttr ? (
      <div key={key} className={leafClassName('', options)} style={{ width: '100%', minWidth: 0 }} {...hit}>
  ...
  if (node.nodeType === 'blog_detail_page') {
    const blogDetailProps = resolveBlogDetailPageProps(node.props);
    const blogDetailCss = sanitizeCompoundWidgetShellCss(css);
    const bCanvas = options.builderCanvas;
    const blogDetailBuilder = Boolean(bCanvas?.blogDetailPage?.onPatchContent);
    const inner = (
      <BlogDetailPage
        key={key}
        style={blogDetailCss}
        {...blogDetailProps}
        builderMode={blogDetailBuilder}
        builderEditable={blogDetailBuilder && !bCanvas?.sectionEditLocked}
        onPatchContent={bCanvas?.blogDetailPage?.onPatchContent}
      />
    );
    return options.builderDataAttr ? (
      <div key={key} className={leafClassName('', options)} style={{ width: '100%', minWidth: 0 }} {...hit}>
  ...
  const blogDetailSectionDef = getBlogDetailSectionDef(node.nodeType);
  if (blogDetailSectionDef) {
    const blogDetailSectionProps = resolveBlogDetailPageProps(node.props);
    const blogDetailSectionCss = sanitizeCompoundWidgetShellCss(css);
    const bCanvas = options.builderCanvas;
    const blogDetailSectionBuilder = Boolean(bCanvas?.blogDetailPage?.onPatchContent);
    const inner = (
      <BlogDetailPage
        key={key}
        style={blogDetailSectionCss}
        {...blogDetailSectionProps}
        sectionOnly={blogDetailSectionDef.sectionKey}
        builderMode={blogDetailSectionBuilder}
        builderEditable={blogDetailSectionBuilder && !bCanvas?.sectionEditLocked}
        onPatchContent={bCanvas?.blogDetailPage?.onPatchContent}
      />
    );
    return options.builderDataAttr ? (
  ...
  const blogSectionDef = getBlogSectionDef(node.nodeType);
  if (blogSectionDef) {
    const blogSectionProps = resolveBlogFullPageProps(node.props);
    const blogSectionCss = sanitizeCompoundWidgetShellCss(css);
    const bCanvas = options.builderCanvas;
    const blogSectionBuilder = Boolean(bCanvas?.blogFullPage?.onPatchContent);
    const blogHubGroupId = String(options.pageId || 'default');
    const inner = (
      <BlogFullPage
        key={key}
        style={blogSectionCss}
        {...blogSectionProps}
        sectionOnly={blogSectionDef.sectionKey}
        blogHubGroupId={blogHubGroupId}
        builderMode={blogSectionBuilder}
        builderEditable={blogSectionBuilder && !bCanvas?.sectionEditLocked}
        onPatchContent={bCanvas?.blogFullPage?.onPatchContent}
        onPatchPost={bCanvas?.blogFullPage?.onPatchPost}
        onAddPost={bCanvas?.blogFullPage?.onAddPost}
      />
  ...
  if (node.nodeType === 'grid_block') {
    const bCanvas = options.builderCanvas;
    const gridBuilder = Boolean(bCanvas?.gridBlock?.onPatchItem);
    return renderAdvancedElementShell(
      key,
      css,
      options,
      ...
      <LiveGridBlock
        ...
        className={leafClassName('', options)}
        builderMode={gridBuilder}
        builderEditable={gridBuilder && !bCanvas?.sectionEditLocked}
        onPatchItem={bCanvas?.gridBlock?.onPatchItem}
        textEditBlurCommitGuard={bCanvas?.textEditBlurCommitGuard}
        featureTabValueSyncGuard={bCanvas?.featureTabValueSyncGuard}
      />
    );
