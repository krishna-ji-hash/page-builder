...
export function shouldStripNeutralDarkCssColor(raw) {
  ...
  if (L < 0.32 && chroma < 0.22) return true;
  return L < 0.52 && chroma < 0.22;
}

/**
 * Buttons with an explicit contrasting text/bg pair (e.g. #111 on #fff) keep authored colors
 * in dark site mode — remapping text to section fg breaks light pills on dark bands.
 * @param {object|null|undefined} style — merged device style_json slice
 */
export function hasAuthoredControlContrast(style) {
  if (!style || typeof style !== 'object') return false;
  const textRaw = style.colors?.text || style.colors?.textColor || style.typography?.color;
  const bgRaw =
    style.colors?.background ||
    style.colors?.backgroundColor ||
    style.background?.backgroundColor;
  if (!textRaw || !bgRaw) return false;
  const textRgb = parseCssColorToRgb(textRaw);
  const bgRgb = parseCssColorToRgb(bgRaw);
  if (!textRgb || !bgRgb) return false;
  const textL = relLuminance255(...textRgb);
  const bgL = relLuminance255(...bgRgb);
  return Math.abs(textL - bgL) >= 0.35;
}
...
/** Pasted Word/Docs often adds white/grey highlight behind each line — strip on render/save. */
export function shouldStripPastedBackgroundCssColor(raw) {
  const val = String(raw || '').trim().toLowerCase();
  if (!val || val === 'transparent' || val === 'inherit' || val === 'initial' || val === 'unset') {
    return true;
  }
  return shouldStripNeutralLightCssColor(val);
}

export function stripPastedBackgroundFromStyleAttr(styleAttr) {
  ...
  for (const part of parts) {
    ...
    if (prop === 'background' || prop === 'background-color') {
      if (shouldStripPastedBackgroundCssColor(val)) continue;
    }
    kept.push(part);
  }
...
export function neutralizeLeafTextCssObject(css, options = {}) {
  ...
  const remapDark = sectionTone === 'dark' || Boolean(options.darkContentMode);
  const remapLight =
  ...
  const textToken =
    sectionTone === 'dark'
      ? 'var(--live-section-muted, var(--color-muted))'
      : options.darkContentMode
        ? 'var(--color-text)'
        : 'var(--live-section-fg, var(--color-text))';
  const remapColor = (key) => {
    const val = next[key];
    if (val == null || val === '') return;
    const c = String(val);
    if (options.darkContentMode && c.includes('--live-section-muted')) {
      next[key] = textToken;
      changed = true;
      return;
    }
...
export function ensureHtmlHooks() {
  ...
  DOMPurify.addHook('afterSanitizeAttributes', (node) => {
    ...
    let next = stripPastedBackgroundFromStyleAttr(old);
    if (neutralizeHardcodedBodyTextColorsActive || remapLightNeutralTextColorsActive) {
      next =
        stripNeutralBodyColorsFromStyleAttr(next, {
