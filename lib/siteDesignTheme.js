...
export function siteThemeDefaultsForNodeType(siteTheme, nodeType) {
  ...
  /** Dark preset: light global `--color-text` is wrong on white strips; rows set `--live-section-fg` when paint is light. */
  const bodyFg =
    t.presetId === 'dark' ? 'var(--live-section-fg, var(--color-text))' : colors.text;
