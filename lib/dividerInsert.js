import { buildDividerCreatePayload, buildDividerStretchVerticalStyleJson } from './dividerDefaults.js';
import {
...
function dividerSideAccentPayload(orient, place) {
  const payload = dividerPayloadForPlacement(orient, place);
  if (orient !== 'vertical') return payload;
  const thickness = Number(payload.props?.thicknessPx) > 0 ? Number(payload.props.thicknessPx) : 2;
  const stretch = buildDividerStretchVerticalStyleJson({ thicknessPx: thickness });
  const desktop = {
    ...stretch.desktop,
    spacing: payload.style_json?.desktop?.spacing,
  };
  return {
    ...payload,
    style_json: { ...payload.style_json, desktop },
  };
}
...
export function resolveDividerInsertPlan(
  ...
  if (place === 'left' || place === 'right') {
    ...
    return {
      ...
      ...(useVertical
        ? dividerSideAccentPayload('vertical', place)
        : dividerPayloadForPlacement(orient, place)),
    };
  }
