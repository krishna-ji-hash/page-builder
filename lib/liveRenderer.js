...
function themedDeviceStyle(node, device, options) {
  ...
  const merged = withResolvedLayoutGap(
    mergeNodeStyleWithSiteTheme(surfaceReady, theme, node.nodeType),
    theme
  );
...
export function renderNode(node, key, options = {}) {
  ...
  if (node.nodeType === 'heading' || node.nodeType === 'text' || node.nodeType === 'rich_text') {
    ...
    leafSectionTone =
      toneFromTree === 'dark' || options.sectionTone === 'dark'
        ? 'dark'
        : toneFromTree || options.sectionTone || null;
  ...
  if (node.nodeType === 'heading') {
      ...
      isProbablyInlineHtml(raw) ? (
        <HeadingTag
          ...
          dangerouslySetInnerHTML={{
            __html:
              sanitizeInlineLeafHtml(raw, { neutralizeHardcodedBodyTextColors: darkContentMode }) || '',
          }}
        />
      ) : (
    ...
    return (
      <div key={key} className="live-leaf-wrap" style={LIVE_LEAF_WRAP_STYLE} {...leafSectionToneAttrs} {...hit}>
        {inner}
      </div>
    );
  }
  ...
  if (node.nodeType === 'text') {
    ...
    const inner = isProbablyInlineHtml(raw) ? (
      <p
        ...
        className={leafClass}
        dangerouslySetInnerHTML={{
          __html: sanitizeInlineLeafHtml(raw, { neutralizeHardcodedBodyTextColors: darkContentMode }) || '',
        }}
      />
    ) : (
  ...
  if (node.nodeType === 'button') {
    ...
    if (onAction) {
      ...
      return shouldAttachLeafClass(options) ? (
        <span key={key} className={leafClassName('', options)} style={{ display: 'inline-block' }} {...hit}>
          {btn}
        </span>
      ) : (
        <ActionButton key={key} style={css} className={leafClassForButton(options, btnClass)} label={content} action={onAction} />
      );
    }
    return (
      <button
