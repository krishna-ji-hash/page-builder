...
function opaqueSolidBackgroundCandidates(css, deviceStyle) {
  return [
    css?.backgroundColor,
    deviceStyle?.background?.backgroundColor,
    deviceStyle?.colors?.backgroundColor,
...
/**
 * Decorative gradients must not flip tone when an opaque solid fill is present underneath.
 * @returns {boolean|null}
 */
function resolveOpaqueSolidBackgroundIsLight(raw, theme) {
  const s = String(raw || '').trim();
  if (!s || isTransparentColor(s) || /gradient/i.test(s)) return null;
  const resolved = resolveContrastPaintSample(s, theme);
  const probe = resolved && !isTransparentColor(resolved) ? resolved : s;
  if (/gradient/i.test(probe)) return null;
  return isCssBackgroundLightSolid(probe);
}

/**
 * Walk opaque solids first, then gradient/image layers. Skips unparseable token vars so a
...
export function resolveSectionBackgroundIsLight({ css, deviceStyle, siteTheme }) {
  const { theme, list } = sectionBackgroundCandidates(css, deviceStyle, siteTheme);
  for (const c of opaqueSolidBackgroundCandidates(css, deviceStyle)) {
    const light = resolveOpaqueSolidBackgroundIsLight(c, theme);
