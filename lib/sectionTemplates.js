...
import {
  ...
  HERO_CONTENT_COLUMN_MOBILE,
  HERO_VISUAL_COLUMN_MOBILE,
  MOBILE_IMAGE_SIZE,
  ROW_MOBILE_STACK_FRAGMENT,
} from '@/lib/responsiveLayoutDefaults.js';
import { defaultSectionLayoutFor } from '@/lib/sectionLayout.js';
import { autoFixTree, validateTree } from '@/lib/builderTree.js';
import { DEFAULT_FEATURE_TABS } from '@/lib/featureTabsDefaults.js';
import { DEFAULT_FAQ_ITEMS } from '@/lib/faqAccordionDefaults.js';
import {
  buildPricingSectionRow,
  buildStatsCounterSectionRow,
  buildContactFormSectionRow,
  buildGetInTouchSectionRow,
  ...
  buildSplitHeroCarouselSectionRow,
  buildTabHeroSectionRow,
  buildWebStorySectionRow,
  buildMapIntegrationSectionRow,
  buildCourierPartnersSectionRow,
} from '@/lib/templateSectionContent.js';
import {
  buildIntegrationBenefitsSectionRow,
  buildIntegrationStepsSectionRow,
  buildIntegrationFeaturesSectionRow,
  buildAiCourierRecommendationSectionRow,
  buildBusinessTypesSectionRow,
} from '@/lib/integrationSectionTemplates.js';
import { buildFaqFullPageSectionRow } from '@/lib/faqFullPageTemplates.js';
import { buildBlogFullPageSectionRow } from '@/lib/blogFullPageTemplates.js';
import {
  BLOG_DETAIL_SECTION_TEMPLATE_BUILDERS,
  buildBlogDetailPageAllSectionsRows,
} from '@/lib/blogDetailSectionTemplates.js';
import { BLOG_SECTION_TEMPLATE_BUILDERS } from '@/lib/blogSectionTemplates.js';
import { HEADER_STARTER_CARDS, buildHeaderStarterRoots } from '@/lib/headerBehavior.js';
...
const B2B_SHIPPING_SERVICES = [
  {
    icon: '🚛',
    title: 'LTL (Less Than Truckload)',
    description:
      'Share truck space for medium freight volumes. Cost-effective B2B movement between warehouses, factories, and distribution hubs with scheduled pickups.',
  },
  {
    icon: '🚚',
    title: 'FTL (Full Truckload)',
    description:
      'Dedicated truck capacity for high-volume lanes. Ideal for bulk pallets, full-container loads, and time-sensitive B2B dispatch across India.',
  },
  {
    icon: '📦',
    title: 'PTL (Part Truckload)',
    description:
      'Flexible part-load options when shipment size sits between LTL and FTL — optimized routing without paying for unused truck space.',
  },
  {
    icon: '✈️',
    title: 'Air Cargo',
    description:
      'Express air freight for urgent B2B shipments, high-value inventory, and tight delivery SLAs between major business hubs.',
  },
  {
    icon: '🏭',
    title: 'Warehouse Fulfillment',
    description:
      'Inbound storage, pick-pack, and outbound dispatch from fulfillment centers — connect inventory to last-mile and line-haul partners.',
  },
  {
    icon: '🌍',
    title: 'Cross-Border B2B',
    description:
      'Documentation-ready international freight for exporters and importers — customs support, lane visibility, and partner coordination.',
  },
  {
    icon: '📅',
    title: 'Appointment Delivery',
    description:
      'Slot-based delivery for retail DCs, plants, and C&F agents. Reduce detention charges and failed delivery attempts on bulk orders.',
  },
  {
    icon: '📍',
    title: 'ePOD & Live Tracking',
    description:
      'Real-time shipment tracking, digital proof of delivery, and audit-friendly logs for finance and operations teams.',
  },
  {
    icon: '🗺️',
    title: 'ODA & Remote Pincodes',
    description:
      'Reach out-of-delivery-area and remote industrial pincodes with mapped serviceability and transparent transit expectations.',
  },
];

const B2B_SECTION_TEXT = 'var(--live-section-fg, var(--token-text-primary))';
const B2B_SECTION_MUTED = 'var(--live-section-muted, var(--token-text-muted))';

function buildB2bShippingServiceCard({ icon, title, description }) {
  return {
    nodeType: 'stack',
    displayName: title,
    props: { meta: { tplRole: 'b2b-service-card' } },
    style_json: {
      desktop: {
        layout: {
          flexDirection: 'column',
        ...
        border: { width: '1px', color: 'rgba(37, 99, 235, 0.16)', radius: '14px' },
        background: {
          backgroundColor: 'var(--token-bg-surface, #ffffff)',
          backgroundImage: 'linear-gradient(180deg, rgba(37, 99, 235, 0.09) 0%, rgba(255, 255, 255, 0) 70%)',
        },
        effects: { boxShadow: '0 10px 28px rgba(15, 23, 42, 0.06)' },
      },
      tablet: {
        layout: {
    ...
    children: [
      {
        ...
        props: { symbol: icon, ariaLabel: title, color: 'var(--color-primary, #2563eb)' },
        style_json: desktop({
          layout: { alignSelf: 'flex-start', flex: '0 0 auto' },
      ...
      },
      {
        ...
        style_json: desktop({
          typography: { fontSize: '17px', fontWeight: '800', lineHeight: '1.3' },
          colors: { textColor: B2B_SECTION_TEXT },
        }),
      },
      {
        nodeType: 'text',
        ...
        props: { text: description },
        style_json: desktop({
          typography: { fontSize: '13px', lineHeight: '1.65' },
          colors: { textColor: B2B_SECTION_MUTED },
        }),
      },
    ],
  };
...
function buildB2bShippingServicesGrid(services) {
  return {
    nodeType: 'stack',
    displayName: 'Services grid',
    props: { meta: { sectionItemsHost: true } },
    style_json: {
      desktop: {
        layout: {
          flexDirection: 'row',
    ...
    children: services.map((item) => buildB2bShippingServiceCard(item)),
  };
}

function buildB2bShippingServicesSectionRow() {
  return {
    nodeType: 'row',
    displayName: 'B2B Shipping Services',
    props: {
      meta: {
        sectionTemplate: 'b2bShippingServices',
        sectionLayout: defaultSectionLayoutFor('b2bShippingServices'),
        tplPolish: true,
      },
    },
    style_json: rowStyle({
      ...
      background: { backgroundColor: 'var(--token-bg-primary, #f8fafc)' },
    }),
    children: [
      {
        nodeType: 'column',
        displayName: 'Services column',
        style_json: columnStyle({ size: { width: '100%', maxWidth: '1120px' } }),
        children: [
          {
            nodeType: 'stack',
            ...
            children: [
                ...
                children: [
                  {
                    ...
                    props: { text: 'Our B2B Shipping Services' },
                    style_json: desktop({
                      typography: {
                        fontSize: '38px',
                        fontWeight: '800',
                  ...
                  },
                  {
                    ...
                    props: {
                      text: 'End-to-end bulk and B2B freight options — from LTL and FTL to air cargo, warehouse fulfillment, and pan-India delivery with live tracking.',
                    },
                    style_json: desktop({
                      typography: { fontSize: '16px', lineHeight: '1.65', textAlign: 'center' },
                      colors: { textColor: B2B_SECTION_MUTED },
                      size: { maxWidth: '760px' },
              ...
              buildB2bShippingServicesGrid(B2B_SHIPPING_SERVICES),
            ],
          },
        ],
      },
...
export const SECTION_TEMPLATES = {
  ...
  hero: [
    {
      ...
      props: {
        meta: {
          ...
          sectionLayout: defaultSectionLayoutFor('hero'),
          tplPolish: true,
        },
      },
      style_json: {
        ...
        mobile: {
          spacing: { padding: '48px 16px' },
          layout: { flexDirection: 'column', gap: 20, alignItems: 'stretch', flexWrap: 'nowrap' },
        },
      },
      children: [
        {
              ...
              children: [
                ...
                },
                {
                  ...
                  style_json: {
                    ...
                    mobile: {
                      typography: { fontSize: '32px', fontWeight: '850', lineHeight: '1.1' },
                    },
                  },
                },
                {
                  ...
                  style_json: {
                    ...
                    mobile: {
                      typography: { fontSize: '16px', lineHeight: '1.55' },
                      size: { maxWidth: '100%' },
                    },
                  },
                },
  ...
  /**
   * B2B shipping services — centered header + 3×3 icon card grid (nine services).
   */
  b2bShippingServices: [buildB2bShippingServicesSectionRow()],

  /**
   * Courier partner logo grid — "OUR NETWORK" header + 5×2 partner cards (Dispatch reference).
   */
  courierPartners: [buildCourierPartnersSectionRow()],

  integrationBenefits: [buildIntegrationBenefitsSectionRow()],
  integrationSteps: [buildIntegrationStepsSectionRow()],
  integrationFeatures: [buildIntegrationFeaturesSectionRow()],
  aiCourierRecommendation: [buildAiCourierRecommendationSectionRow()],
  businessTypes: [buildBusinessTypesSectionRow()],
  ...
  faqFullPage: [buildFaqFullPageSectionRow()],
  /** Full blog page — hero, tabs, featured, grid, guides, newsletter, CTA. */
  blogFullPage: [buildBlogFullPageSectionRow()],
  blogDetailPage: buildBlogDetailPageAllSectionsRows(),
  ...Object.fromEntries(
    Object.entries(BLOG_DETAIL_SECTION_TEMPLATE_BUILDERS).map(([key, build]) => [key, [build()]])
  ),
  ...Object.fromEntries(
    Object.entries(BLOG_SECTION_TEMPLATE_BUILDERS).map(([key, build]) => [key, [build()]])
  ),
  ...
  getInTouch: [buildGetInTouchSectionRow()],
  ...
  splitHeroCarousel: [buildSplitHeroCarouselSectionRow()],
  tabHero: [buildTabHeroSectionRow()],
...
for (const card of HEADER_STARTER_CARDS) {
  SECTION_TEMPLATES[card.id] = buildHeaderStarterRoots(card.id, buildHeaderSectionTree);
}
 ...
 * @returns {object[]}
 */
export function buildHeaderStarterSectionRoots(starterId) {
  return buildHeaderStarterRoots(starterId, buildHeaderSectionTree);
}
